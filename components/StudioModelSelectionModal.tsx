...
const StudioModelSelectionModal: React.FC<StudioModelSelectionModalProps> = ({ event, onClose, onSuccess }) => {
  ...
  const handleBatchJoin = async () => {
    ...
    alert(`成功！已为 ${selectedIds.size} 位模特申请加入活动：${event.name}`);
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-8 border-b bg-gray-50/50">
          <div className="flex justify-between items-start">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-2xl font-black text-gray-800">工作室批量报名</h3>
                <p className="text-sm text-gray-500 mt-1">
                  活动：<span className="font-bold text-indigo-600">{event.name}</span>
                </p>
              </div>
            </div>
          ...
          <div className="mt-8 flex items-center space-x-4">
            ...
            <button 
              ...
              className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap"
            >
              {selectedIds.size === availableModels.filter(m => m.isAvailable).length ? '取消全选' : '全选可用'}
            </button>
          </div>
        </div>
          ...
          {availableModels.map((model) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-[10px] uppercase font-bold ${model.isAvailable ? 'text-green-500' : 'text-gray-400'}`}>
                  {model.isAvailable ? '可报名' : '已在活动中'}
                </p>
              </div>
              {model.isAvailable && (
        ...
        <div className="p-8 border-t bg-gray-50/50 flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0">
          <div className="text-center md:text-left">
            <p className="text-sm text-gray-500 font-medium">已选择 <span className="text-indigo-600 font-black text-lg">{selectedIds.size}</span> 位模特</p>
            <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-0.5">预计增加总体流量曝光约 +15%</p>
          </div>
          <div className="flex items-center space-x-4 w-full md:w-auto">
            <button 
              onClick={onClose}
              className="flex-1 md:flex-none px-8 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-200 transition-all"
            >
              取消
            </button>
            <button 
              ...
              className={`flex-1 md:flex-none px-12 py-3 rounded-xl font-black shadow-xl transition-all flex items-center justify-center space-x-2 ${
